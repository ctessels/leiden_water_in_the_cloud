...
data_folder = Path(__file__).resolve().parent / 'sensor_data'
...
valkenburg_precip_df.to_csv(data_folder / 'valkenburg_precipitation.csv', index=False)
...
with open(data_folder / "voorschoten_weerdata.csv", "w", encoding="utf-8") as f:
    f.write(header_lines[18].replace('# ','') + "\n")
    for row in data_part.splitlines():
        f.write(row + "\n")

# Format dates
voorschoten_weather_df = pd.read_csv(data_folder / "voorschoten_weerdata.csv")
...
voorschoten_weather_df.to_csv(data_folder / 'voorschoten_weerdata.csv', index=False)
...
conn = sqlite3.connect(data_folder / 'database.db')
cursor = conn.cursor()
